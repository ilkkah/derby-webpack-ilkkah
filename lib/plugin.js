...
function plugin(app, options) {
  ...
  app.on('htmlDone', (page) => {
    ...
    if (page.res.locals.webpack) {
      rootDir = page.res.locals.webpack.options.publicPath;
      assets = middlewareAssets(page);
    } else {
      assets = readManifestAssets('./public/manifest.json');
    }
    const scriptTags = assets.map(path => `<script ${scriptCrossOrigin ? 'crossorigin ' : ''}src="${join(rootDir, path)}" type="text/javascript"></script>`)
      .join('\n');
